...
r-install: prepare-pixi
	$(PIXI_RUN) bash -lc ' \
		...
		test -f "$$CONDA_PREFIX/include/glpk.h" || true; \
		test -f "$$CONDA_PREFIX/lib/libglpk.so" || test -f "$$CONDA_PREFIX/lib/libglpk.a" || true; \
		test -f "$$CONDA_PREFIX/lib/liblzma.so" || test -f "$$CONDA_PREFIX/lib/liblzma.so.5" || true; \
